...
TRam::TRam(TScreen *s)
{
    ...
    partageRam.status.valveManuelleGB = 0;
    partageRam.status.valveManuellePB = 0;
    ...
    partageRam.alarmes.tempsGenNGB = 1;
    partageRam.alarmes.tempsGenNPB = 1;
    partageRam.alarmes.tempsRepNGB = 1;
    partageRam.alarmes.tempsRepNPB = 1;
    partageRam.alarmes.niveauANGB = 70;
    partageRam.alarmes.niveauANPB = 70;
...
void TRam::setPompe(bool onOff)
{
    ...
    // module 4 DO1
    sprintf(bufCmd, "#041%d%02d", 0, (!this->getDebordementGrosBassin() || !this->getDebordementPetitBassin()) ? 0 : onOff);
    bufCmd[7] = '\x0D';
...
double TRam::getNiveauGrosBassin(void)
{
    ...
    partageRam.lock.take();
    retour = filtreGrosBassin.filtre(partageRam.status.niveauGrosBassin);
    partageRam.lock.release();
...
double TRam::getNiveauPetitBassin(void)
{
    ...
    partageRam.lock.take();
    retour = filtrePetitBassin.filtre(partageRam.status.niveauPetitBassin);
    partageRam.lock.release();
...
void TRam::setManValveGB(double ouverturePourcent)
{
    partageRam.lock.take();
    partageRam.status.valveManuelleGB = ouverturePourcent;
    partageRam.lock.release();
}

void TRam::setManValvePB(double ouverturePourcent)
{
    partageRam.lock.take();
    partageRam.status.valveManuellePB = ouverturePourcent;
    partageRam.lock.release();
}

double TRam::getManValveGB(void)
{
    double retour;

    partageRam.lock.take();
    retour = partageRam.status.valveManuelleGB;
    partageRam.lock.release();

    return retour;
}

double TRam::getManValvePB(void)
{
    double retour;

    partageRam.lock.take();
    retour = partageRam.status.valveManuellePB;
    partageRam.lock.release();
