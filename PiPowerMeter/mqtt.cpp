...
TMqtt::TMqtt(void *shared, const char *id, const char *host, int port) : mosqpp::mosquittopp(id)
{
	...
	mqttTable["alarmes/etats/CNX_POW"] = &TMqtt::messageCNX_POW;
...
{
	//screen->dispStr(1, 13, "on_subcribe");
}

void TMqtt::messageCNX_POW(string val)
{
	if (val == "ACK")
	{
		partage->setTimeoutChecker(false);
		partage->setChangeTable(TPartage::ALARM, false);
		publish(NULL, "ram/alarmes/etats/CNX_POW", 0, "");
	}
}
